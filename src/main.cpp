...
Vec3<float> barycentric(Vec3<float> a, Vec3<float> b, Vec3<float> c,
                        Vec3<float> P) {
  ...
  for (int i = leftOfBox; i <= rightOfBox; i++) {
    for (int j = bottomOfBox; j <= topOfBox; j++) {
      ...
      if (bary_coords.x >= -0.001 && bary_coords.y >= -0.001 &&
        ...
        // current coordinate. We can do this by using barycentric coordinates
        // to interpolate
        float z =
...
//     float scale_right = ((float)(y - (y > v1.y ? v1.y : v0.y)) / (d_right.y
//     == 0 ? 1 : d_right.y)); int x_left = v0.x + d_left.x * scale_left; int
//     x_right = (y > v1.y ? v1.x : v0.x) + d_right.x * scale_right;
//
//     if (std::abs(x_left - x_right) > 0) { // fix random dot being drawn when
//     distance = 0
...
int main(int argc, char **argv) {
  ...
  //
  // Vec3<float> t0[3] = {Vec3<float>(10, 70, 0),   Vec3<float>(60, 70, 0),
  // Vec3<float>(60, 140, 0)}; Vec3<float> t1[3] = {Vec3<float>(60, 70, 0),
  // Vec3<float>(110, 70, 0),   Vec3<float>(60, 140, 0)}; draw_triangle(t0[0],
  // t0[1], t0[2], image, green, zbuffer); draw_triangle(t1[0], t1[1], t1[2],
  // image, white, zbuffer);
